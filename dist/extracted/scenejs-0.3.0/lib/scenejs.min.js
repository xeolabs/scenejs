...
++j){o[d[j]]=o[d[j]]||{};o=o[d[j]];}}},newScope:function(_parent,_fixed){var parent=_parent;var data={};var fixed=_fixed||(_parent?_parent.isfixed():false);return{put:function(key,value){data[key]=value;},get:function(key){var value=data[key];if((value==0)||value){return value;}if(!parent){return null;
}return parent.get(key);},isfixed:function(){return fixed;}};},getNodeConfig:function(args){if(args.length==0){throw new SceneJs.exceptions.InvalidNodeConfigException("Invalid node parameters: should be a configuration followed by zero or more child nodes");}var result={};var a0=args[0];if(a0 instanceof Function){result.getParams=a0;
...
}while(program.vertexShaders.length>0){program.context.deleteShader(program.vertexShaders.pop());}program.context.deleteProgram(program.program);}};this.loadProgram=function(_cfg){if(!ctx.canvas){throw new SceneJs.exceptions.NoCanvasActiveException("No canvas active");}var programId=ctx.canvas.canvasId+":"+_cfg.type;
var program=programs[programId];if(program){return programId;}var context=ctx.canvas.context;program={canvas:ctx.canvas,context:context,programId:programId,program:context.createProgram(),fragmentShaders:[],vertexShaders:[],setters:_cfg.setters,binders:_cfg.binders};for(var i=0;i<_cfg.fragmentShaders.length;
...
}var setter=activeProgram.setters[name];if(setter){setter.call(this,ctx.canvas.context,activeProgram.getVarLocation,value);}};this.setVars=function(v){if(!activeProgram){throw new SceneJs.exceptions.NoShaderActiveException("No shader active");}for(var key in activeProgram.setters){activeProgram.setters[key].call(this,ctx.canvas.context,activeProgram.getVarLocation,v.vars[key]);
}vars=v;};this.getVars=function(){return vars;};this.bindVertexBuffer=function(buffer){if(!activeProgram){throw new SceneJs.exceptions.NoShaderActiveException("No shader active");}activeProgram.binders.bindVertexBuffer.call(this,ctx.canvas.context,activeProgram.getVarLocation,buffer);};this.bindNormalBuffer=function(buffer){if(!activeProgram){throw new SceneJs.exceptions.NoShaderActiveException("No shader active");
}activeProgram.binders.bindNormalBuffer.call(this,ctx.canvas.context,activeProgram.getVarLocation,buffer);};this.deactivateProgram=function(){if(!activeProgram){throw new SceneJs.exceptions.NoShaderActiveException("No shader active");}ctx.canvas.context.flush();activeProgram=null;ctx.canvas.context.useProgram(null);
};this.deletePrograms=function(){for(var programId in programs){deleteProgram(programs[programId]);}programs={};activeProgram=null;vars={};};};}};this.loadProgram=function(){return ctx.programs.loadProgram(cfg);};this.activateProgram=function(programId){ctx.programs.activateProgram(programId);};this.getActiveCanvasId=function(){return ctx.canvas.canvasId;
...
SceneJs.utils.visitChildren(cfg,scope);backend.popLights(lights.length);};};SceneJs.material=function(scenejs){var cfg=SceneJs.utils.getNodeConfig(arguments);var backend;var cloneColor=function(v){return v?{r:v.r||0,g:v.g||0,b:v.b||0,a:v.a||1}:{r:0,g:0,b:0};};var material;return function(scope){if(!backend){backend=SceneJs.backends.getBackend("material");
}if(!material||!cfg.fixed){var params=cfg.getParams(scope);material={ambient:cloneColor(params.ambient),diffuse:cloneColor(params.diffuse),specular:cloneColor(params.specular),shininess:cloneColor(params.shininess)};}var saveMaterial=backend.getMaterial();backend.setMaterial(material);SceneJs.utils.visitChildren(cfg,scope);
backend.setMaterial(saveMaterial);};};SceneJs.scope=function(){var cfg=SceneJs.utils.getNodeConfig(arguments);var childScope;return function(scope){if(!childScope||!cfg.fixed||!scope.isfixed()){childScope=SceneJs.utils.newScope(scope,cfg.fixed);var params=cfg.getParams(scope);if(params){for(var key in params){childScope.put(key,params[key]);
}}}SceneJs.utils.visitChildren(cfg,childScope);};};SceneJs.generator=function(){var cfg=SceneJs.utils.getNodeConfig(arguments);return function(scope){if(cfg.fixed){throw new SceneJs.exceptions.InvalidNodeConfigException("generator node must be configured with a function");}var params=cfg.getParams(scope);
while(params){var childScope=SceneJs.utils.newScope(scope);for(var key in params){childScope.put(key,params[key]);}SceneJs.utils.visitChildren(cfg,childScope);params=cfg.getParams(scope);}};};SceneJs.scalarInterpolator=function(){var cfg=SceneJs.utils.getNodeConfig(arguments);var NOT_FOUND=0;var BEFORE_FIRST=1;
...
default:break;}};update();var childScope=SceneJs.utils.newScope(scope,cfg.fixed);childScope.put(params.output,outputValue);SceneJs.utils.visitChildren(cfg,childScope);};};SceneJs.asset=function(){var cfg=SceneJs.utils.getNodeConfig(arguments);var backend;var loading=false;return function(scope){var params=cfg.getParams(scope);
...
});}else{if(assetNode){assetNode.call(this,scope);loading=false;}}SceneJs.utils.visitChildren(cfg,scope);};};SceneJs.assetBackend=function(cfg){if(!cfg.type){throw new SceneJs.exceptions.NodeConfigExpectedException("Asset backend config missing: type");}if(!cfg.parse){throw new SceneJs.exceptions.NodeConfigExpectedException("Asset backend config missing: parser");
}return new (function(){this.type=cfg.type;var ctx;this.install=function(_ctx){ctx=_ctx;if(!ctx.assets){ctx.assets=new function(){var date=new Date();var importers={};var entries={};var evictionCountdown=1000;var evict=function(){if(--evictionCountdown==0){evictionCountdown=1000;}};this.installImporter=function(cfg){importers[cfg.type]={type:cfg.type,parse:cfg.parse};
...
}catch(e){}head.removeChild(script);};head.appendChild(script);};this.getAsset=function(uri){var entry=entries[uri];if(entry){entry.timeToLive++;evict();return entry.node;}return null;};this.loadAsset=function(proxy,uri,type,callback){var importer=importers[type];if(!importer){throw'No asset node backend registered for file type: "'+type+'"';
}var callbackName="callback"+date.getTime();jsonp(proxy+"?callback="+callbackName+"&uri="+uri,function(assetNode){entries[uri]={node:assetNode,timeToLive:1000};callback(assetNode);},callbackName);};this.clearAssets=function(){entries={};};};}ctx.assets.installImporter(cfg);};this.getAsset=function(uri){return ctx.assets.getAsset(uri);
};this.loadAsset=function(proxy,uri,callback){return ctx.assets.loadAsset(proxy,uri,cfg.type,callback);};this.reset=function(){ctx.assets.clearAssets();};})();};SceneJs.backends.installBackend((function(){return SceneJs.assetBackend({type:"asset.dae",parse:function(){return SceneJs.objects.teapot({});
}});})());SceneJs.backends.installBackend((function(){return SceneJs.assetBackend({type:"asset.js",parse:function(location){var req=new Remote.JSON(location);req.callback("functionToCallOnScriptLoad");return SceneJs.objects.teapot({});}});})());