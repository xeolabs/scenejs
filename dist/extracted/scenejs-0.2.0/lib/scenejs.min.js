...
SceneJs.utils.visitChildren(cfg,scope);backend.setMaterial(saveMaterial);};};SceneJs.scope=function(){var cfg=SceneJs.utils.getNodeConfig(arguments);var childScope;return function(scope){if(!childScope||!cfg.fixed||!scope.isfixed()){childScope=SceneJs.utils.newScope(scope,cfg.fixed);var params=cfg.getParams(scope);
...
}var params=cfg.getParams(scope);while(params){var childScope=SceneJs.utils.newScope(scope);for(var key in params){childScope.put(key,params[key]);}SceneJs.utils.visitChildren(cfg,childScope);params=cfg.getParams(scope);}};};SceneJs.scalarInterpolator=function(){var cfg=SceneJs.utils.getNodeConfig(arguments);
var NOT_FOUND=0;var BEFORE_FIRST=1;var AFTER_LAST=2;var FOUND=3;var params;var outputName;var outputValue;return function(scope){if(!params){params=cfg.getParams(scope);if(!cfg.fixed){throw new SceneJs.exceptions.UnsupportedOperationException("Dynamic configuration of interpolators is not supported");
}if(!params.input){throw"scalarInterpolator input parameter missing";}if(!params.output){throw"scalarInterpolator output parameter missing";}if(params.keys){if(!params.values){throw"scalarInterpolator keys supplied but no values - must supply a value for each key";}}else{if(params.values){throw"scalarInterpolator values supplied but no keys - must supply a key for each value";
...
default:break;}};update();var childScope=SceneJs.utils.newScope(scope,cfg.fixed);childScope.put(params.output,outputValue);SceneJs.utils.visitChildren(cfg,childScope);};};