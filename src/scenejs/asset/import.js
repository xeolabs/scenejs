/** Asynchronously imports a subgraph cross-domain. This node is configured with the location of a file
 ...
 *
 * Can be configured with child nodes to visit while the import is in progress, which will be replaced by
 * the imported subgraph.
 */
SceneJS.import = function() {
    var cfg = SceneJS._utils.getNodeConfig(arguments);
    if (!cfg.fixed) {
        throw new SceneJS.exceptions.UnsupportedOperationException
                ("Dynamic configuration of SceneJS.import nodes is not supported");
    }
    var params;
                ...
                if (!params) {
                    ...
                    if (!params.uri) {
                        throw new SceneJS.exceptions.NodeConfigExpectedException
                                ("Mandatory SceneJS.import parameter missing: uri");
                    }                 
                }
                        ...
                        process = backend.loadAsset(// Process killed automatically on error or abort
                                ...
                                function(msg) { // onError - backend has killed process
                                    ...
                                    logging.getLogger().error(
                                            "SceneJS.import failed - " + msg + " - uri: " + params.uri);
                                });
